...
export default function Sidebar(props: SidebarProps) {
  ...
  const stats: [string | number, string][] = hasParcels
    ...
    : [[meta?.n_occurrences || 0, "Showings"],
       ...
       [hotspots[0]?.provincia?.toLowerCase() || "—", "Top target"]];
    ...
    <aside id="sidebar">
      ...
      <section className="step">
        ...
        {models.length > 0 && <>
          ...
          <div className="t-btns">
            {([{ key: null, label: "All" }, ...models.map((m) => ({ key: m.key, label: m.key }))] as const).map((b) => (
              <button key={b.key || "all"} className={(target || "") === (b.key || "") ? "on" : ""}
                onClick={() => setTarget(b.key)}>
                {b.key && <span className="dot" style={{ background: cinfo(b.key).color }} />}{b.label}
              </button>
            ))}
          </div>
      ...
      {drillParcels.length > 0 && (
        <section className="step">
          ...
          <p className="explain">Cadastral parcels scored for acquisition — prospectivity, price, and how
            much is <b>already legally claimed</b>. <span style={{ color: "var(--muted)" }}>· hotspot #{drill!.rank}</span></p>
      ...
      {(data?.occ || data?.workings) && (
        <details className="step">
          <summary style={{ cursor: "pointer", color: "var(--muted)", fontSize: 12, fontWeight: 600 }}>Reference layers</summary>
